...
export const descargarPresupuestoPDF = (presupuesto, proyecto) => {
  ...
  // Mapa de notas / observaciones para la simbología
  const footnotesList = [];
    ...
    items.forEach((c, idx) => {
      ...
      html += `
        <tr style="border-bottom: 1px solid #e2e8f0; page-break-inside: avoid;">
          <td style="padding: 5px 4px; text-align: center; font-weight: 700; color: #64748b; font-size: 8.5px; vertical-align: middle;">${c.no || '•'}</td>
    ...
    <div style="border-bottom: 2px solid #0f172a; padding-bottom: 6px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: flex-end;">
      <div>
        ...
        <div style="font-size: 9px; color: #475569; margin-top: 2px; font-weight: 700;">
          Gestoría de Construcción &nbsp;|&nbsp; Gestión Integral Urbana
        </div>
      </div>
    ...
    <div style="page-break-inside: avoid; margin-top: 8px; margin-bottom: 14px;">
      <div style="display: flex; flex-direction: column; gap: 4px; font-size: 7.5px; color: #000000; line-height: 1.35; text-align: center;">
        ${infoAdicional.documentosTecnicos ? `
          <div>
            <strong style="color: #000000;">DOCUMENTOS TECNICOS NECESARIOS:</strong>
            <span style="color: #dc2626; font-weight: 700;"> ${infoAdicional.documentosTecnicos}</span>
          </div>
        ` : ''}
        ${infoAdicional.documentosLegales ? `
          <div>
            <strong style="color: #000000;">DOCUMENTOS LEGALES NECESARIOS:</strong>
            <span> ${infoAdicional.documentosLegales}</span>
          </div>
        ` : ''}
        ${infoAdicional.derechosGastos ? `
          <div>
            <strong style="color: #000000;">DERECHOS Y GASTOS:</strong>
            <span> ${infoAdicional.derechosGastos}</span>
          </div>
        ` : ''}
        ${infoAdicional.formaPago ? `
          <div>
            <strong style="color: #000000;">FORMA DE PAGO:</strong>
            <span> ${infoAdicional.formaPago}</span>
          </div>
        ` : ''}
        ${infoAdicional.exclusiones ? `
          <div>
            <strong style="color: #000000;">LOS TRABAJOS NO INCLUYEN:</strong>
            <span> ${infoAdicional.exclusiones}</span>
          </div>
        ` : ''}
        ${infoAdicional.notas ? `
          <div>
            <strong style="color: #000000;">NOTAS:</strong>
            <span> ${infoAdicional.notas}</span>
          </div>
        ` : ''}
        <div style="color: #dc2626; font-weight: 800; font-size: 8px; margin-top: 2px;">PRECIOS MÁS I.V.A. SI SE REQUIERE</div>
      </div>
    </div>
