...
const getConcepto = (clave) => CATALOGO_CONCEPTOS.find(c => c.clave === clave)
  ...
  return (
    <aside className="sidebar">
      <div className="sidebar-logo">
        <div className="wordmark">GestorCons</div>
...
function Catalogo() {
  ...
  const filtered = CATALOGO_CONCEPTOS.filter(c =>
    ...
    <div>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <div className="page-subtitle">{CATALOGO_CONCEPTOS.length} conceptos disponibles para presupuestos</div>
        </div>
      </div>
...
// ─── COTIZACIONES ─────────────────────────────────────────────────────────────
function Cotizaciones({ cotizaciones, setCotizaciones }) {
  ...
  const buscarConcepto = (q) => {
    ...
    setResultados(CATALOGO_CONCEPTOS.filter(c =>
      c.clave.toLowerCase().includes(q.toLowerCase()) ||
      c.descripcion.toLowerCase().includes(q.toLowerCase())
    ).slice(0, 6))
  }
    ...
    <div>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <div className="page-subtitle">{cotizaciones.length} cotizaciones registradas</div>
        </div>
...
const initialPresupuestosDB = [
  {
    ...
    derechos:   { planos: { cantidad:'0', precioUnitario:'0'  }, hrsHombre: { horas:'3',  precioPorHora:'350' }, oficina: { horas:'5',  precioPorHora:'200' } },
  ...
  },
  {
    ...
    derechos:   { planos: { cantidad:'0', precioUnitario:'0'   }, hrsHombre: { horas:'2', precioPorHora:'350' }, oficina: { horas:'3', precioPorHora:'200' } },
    fecha: fmt(addDays(hoy, -10)), clienteId: 2,
  },
]

function FormNuevoPresupuesto({ onGuardar, onCancelar }) {
  ...
  const [derechos,   setDerechos]     = useState(emptyCosto())
  const [guardado, setGuardado]       = useState(false)

  const resH = calcSeccion(honorarios)
  const resD = calcSeccion(derechos)
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:20, alignItems:'start' }}>
        <div>
          ...
          <div className="card" style={{ marginBottom:16 }}>
            ...
            <CalcBloque titulo="Derechos" color="#1A5276" datos={derechos} onChange={setDerechos} />
          </div>
        </div>
        ...
        <div style={{ position:'sticky', top:20 }}>
          <div className="card" style={{ marginBottom:12 }}>
            ...
            <div style={{ marginBottom:12 }}>
              ...
              {[['📐 Planos', resD.planos],['👷 Hrs. Hombre', resD.hrs],['🏢 Oficina', resD.oficina]].map(([l,v])=>(
                <div key={l} style={{ display:'flex', justifyContent:'space-between', fontSize:12, padding:'4px 0', color:'var(--text-2)' }}>
                  <span>{l}</span><span style={{ fontFamily:'DM Mono' }}>{money(v)}</span>
                </div>
              ))}
          ...
          {guardado ? (
            ...
            <button className="btn btn-primary w-full" onClick={handleGuardar} disabled={!titulo} style={{ opacity:!titulo?0.45:1, justifyContent:'center', padding:'11px', fontSize:14 }}>
...
function VistaPresupuesto({ p, onCerrar }) {
  ...
  const resD = calcSeccion(p.derechos)
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 320px', gap:16 }}>
        ...
        <div className="card">
          ...
          <CostRow label="📐 Planos" value={resD.planos} />
          <CostRow label="👷 Hrs. Hombre" value={resD.hrs} />
          <CostRow label="🏢 Oficina" value={resD.oficina} />
...
function Presupuestos() {
  ...
  const [presupuestos, setPresupuestos] = useState(initialPresupuestosDB)
  ...
  if (tab === 'nuevo') return <FormNuevoPresupuesto onGuardar={guardarNuevo} onCancelar={()=>setTab('catalogo')} />
  ...
  const catalogoFiltrado = CATALOGO_CONCEPTOS.filter(c =>
    c.clave.toLowerCase().includes(q.toLowerCase()) ||
    c.descripcion.toLowerCase().includes(q.toLowerCase())
  )
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {active === 'cotizaciones'  && <Cotizaciones cotizaciones={cotizaciones} setCotizaciones={setCotizaciones} />}
